...
フロントエンドとバックエンドを分けています。
XREA Freeなど常駐に対応してないVPSではバックエンドは動かせないため、バックエンドはRenderやMWSのような
...
2. 公開ディレクトリ(例: `public_html/`)にそのままアップロード
3. ブラウザで自分のサーバーのURLを開く
4. ログイン画面で以下を入力
   - **バックエンドURL**: 手順2で控えたバックエンドのURL
   - **共有シークレット**: バックエンドで設定した `APP_SECRET`(設定していなければ空欄)
   - **Botトークン**: 手順1のBotトークン
5. 「ログイン」を押すと、サーバー一覧が表示されます
...
にアクセスしてもらうとフロントエンドは公開していますので使用することができます
またバックエンドサーバーを自分で立ち上げれないなどサポートが必要な方はDiscord(i8a_.)にてサポートを行なっています
